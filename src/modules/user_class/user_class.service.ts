...
export class UserClassService {
  ...
  async create(
    ...
    if (await this.checkExistStudentInClass(createUserClassDto.userId, createUserClassDto.classId)) {
    ...
    const classData = await this.classService.findOne(createUserClassDto.classId);  
    if (!classData) {
    ...
    if (!result) {
      throw new NotFoundException('Failed to create class registration information');
    } 
    return result
  }

  async checkExistStudentInClass(userId: number, classId: number) { 
    return await this.userClassRepository.createQueryBuilder('user_class')
  ...
  async update(
    ...
    const classData = await this.classService.findOne(updateUserClassDto.classId);
    if (!classData) {
      throw new NotFoundException('Class information is not found');
    }
    ...
    if (this.checkExistStudentInClass(updatedUserClass.user.id, updatedUserClass.class.id)) { 
      throw new BadRequestException('Student already exists in the class');
    }
    const result = await this.userClassRepository.save(updatedUserClass);
    if(!result) {
      throw new NotFoundException('Failed to update class registration information');
    }
    return result;
  }
